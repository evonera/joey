...
export function TelegramPanel() {
  ...
  async function connect() {
    ...
    if (allowedUserIds.length === 0) { toast.error("Add at least one Telegram user ID. An empty allowlist denies everyone."); return; }
    setSaving(true);
  ...
  return <section className="rounded-xl border bg-white p-6 shadow-sm dark:bg-zinc-900">
    ...
    <p className="mt-1 text-xs text-zinc-500">Connect a tenant-owned bot. Only private DMs from the listed user IDs are admitted; bots and group messages are ignored. The token is encrypted and never shown again.</p>
    ...
    <div className="mt-4 grid gap-3 md:grid-cols-2">
      ...
      <Input value={users} onChange={(event) => setUsers(event.target.value)} placeholder="Allowed Telegram user IDs, comma separated (required)" />
    </div>
