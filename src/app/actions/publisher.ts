...
export async function publishDraft(draftId: string): Promise<{ success?: boolean; error?: string; status?: string }> {
    try {
        ...
        if (pkg) {
            ...
            const res = await publishContentPackage(draftId, tenantId);
            if (!res.success) {
