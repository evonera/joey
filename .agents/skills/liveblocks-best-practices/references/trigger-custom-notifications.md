...
function AlertNotification(props: InboxNotificationCustomKindProps<"$alert">) {
  ...
  return (
    <InboxNotification.Custom {...props} title={title} aside={<InboxNotification.Icon>❗</InboxNotification.Icon>}>
      {message}
    </InboxNotification.Custom>
  );
}
...
function Notification({ inboxNotification }) {
  return <InboxNotification inboxNotification={inboxNotification} kinds={{ $alert: AlertNotification }} />;
}
```
